...
router.get('/', async (ctx, next) => {
  ...
  ctx.response.body = await getItems(getQueryObj({status:1, ...q})); // q.page, q.pSize, q.keyword, q.order
});
router.get('/all', async (ctx, next) => {
  var q = await getSidQuery({ctx, isItems:1})
  ctx.response.body = await getItems(getQueryObj(q)); // q.page, q.pSize, q.keyword, q.order
});
